...
public interface BikeRepository extends JpaRepository<BikeEntity, UUID> {
    ...
    // 사용자의 모든 활성 바이크 조회
    List<BikeEntity> findByUserEntityIdAndDeletedAtIsNullOrderByIsRepresentativeDescCreatedAtDesc(UUID userId);

    // 특정 활성 바이크 조회
    Optional<BikeEntity> findByIdAndDeletedAtIsNull(UUID id);

    // 대표 바이크 조회
    Optional<BikeEntity> findByUserEntityIdAndIsRepresentativeTrueAndDeletedAtIsNull(UUID userId);
