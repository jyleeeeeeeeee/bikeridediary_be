...
public class BikeModelController {
    ...
    @Operation(summary = "모델명 목록 조회", description = "특정 제조사의 고유 모델명 + 타입 목록 반환")
    @GetMapping("/manufacturers/{manufacturerName}/models")
    public ResponseEntity<ApiResponse<List<BikeModelNameResponse>>> getModelNames(
            @PathVariable String manufacturerName) {
        return ResponseEntity.ok(ApiResponse.ok(bikeModelService.getModelNames(manufacturerName)));
    }

    @Operation(summary = "모델 상세 조회", description = "특정 제조사의 특정 모델에 대한 연식별 상세 정보 반환")
    @GetMapping("/manufacturers/{manufacturerName}/models/{modelName}")
    public ResponseEntity<ApiResponse<List<BikeModelResponse>>> getModelDetails(
            @PathVariable String manufacturerName,
            @PathVariable String modelName) {
        return ResponseEntity.ok(ApiResponse.ok(bikeModelService.getModelDetails(manufacturerName, modelName)));
    }

    @Operation(summary = "목록에 없는 바이크 API로 조회, DB 등록", description = "API-Ninjas에서 특정 제조사의 모델 데이터 조회, 저장")
    @PostMapping("/sync/{manufacturerName}/{modelName}")
    public ResponseEntity<ApiResponse<Map<String, Integer>>> saveModel(
            @PathVariable String manufacturerName,
            @PathVariable String modelName) {
        int count = bikeModelService.syncManufacturerModels(manufacturerName, modelName);
